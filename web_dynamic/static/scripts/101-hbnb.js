$(document).ready(function () {
      ...
      success: function (data) {
        data.forEach(function (place) {
          ...
          $.get('http://0.0.0.0:5001/api/v1/places/' + place.id + '/reviews', function (data) {
           ...
           ' Bathroom</p> </div> </div>' + '<div class="description">' + place.description + '</div>' + '<div class="reviews">' +
           '<h2>Reviews<span>&#9;&#9;show</span></h2>' + '<ul>' + text + '</ul></div></article>';
  ...
  $('.places').on('click', '.reviews > h2 > span', function () {
    $(this).parents().children('ul').toggleClass('show');
